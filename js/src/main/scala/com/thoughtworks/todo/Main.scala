...
@JSExportTopLevel("Main") object Main {
  ...
  object Models {
    ...
    val autoSave: Binding[Unit] = Binding {
      save(allTodos.all.bind)
    }
    autoSave.watch()
  ...
  @dom def todoListItem(todo: Todo): Binding[Node] = {
    ...
    <li class={s"${if (todo.completed) "completed" else ""} ${if (editingTodo.bind.contains(todo)) "editing" else ""}"}>
      <div class="view">
        ...
        <label ondblclick={_: Event => editingTodo.value = Some(todo); editInput.focus()}>
          {todo.title}
        </label>
  ...
  @dom def footer: Binding[Node] = {
    ...
    <footer class="footer" style:display={if (allTodos.length.bind == 0) "none" else ""}>
      <span class="todo-count">
        <strong>
          {active.items.length.bind.toString}
        </strong>{if (active.items.length.bind == 1) "item" else "items"}
        left
      </span>
      <ul class="filters">
  ...
  implicit def makeIntellijHappy(x: NodeBuffer): Binding[BindingSeq[Node]] = throw new AssertionError("This should never execute.")
